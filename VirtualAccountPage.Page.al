            ...
            group(General)
            {
                ...
                field("Bal. Account No."; Rec."Bal. Account No.")
                {
                    ...
                    begin
                        ...
                        Rec.fullName := SelectedCustomer.Name;
                        if Rec.Email = '' then
    ...
    actions
    {
        ...
        area(Processing)
        {
            ...
            action("Retrieve Payments")
            {
                ...
                begin
                    actionType := 'Verify';
                    if Dialog.Confirm('Retrieve payments for account number ' + Format(Rec.accountNumber) + '. Payments will be posted using Account Type/No. and Bal. Account Type/No.', true, false) then
                        SendDataToAPI(rec.ID);
                            ...
                            for Index := 0 to PaymentsArray.Count() - 1 do begin
                                if (PaymentsArray.Get(index, PaymentArrayToken)) then begin
                                    ...
                                    if not paymentsRecord.FindSet then begin
                                        ...
                                        genraljournalline."Document No." := 'VA-' + Format(paymentsRecord.Id);
